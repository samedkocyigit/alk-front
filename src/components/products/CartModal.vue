<template>
  <div class="absolute bg-white shadow-d-10 top-10 right-0 w-[380px] h-fit rounded-lg border-[1px] z-[3]">
    ...
    <div class="w-full p-1 px-2 h-[300px] overflow-y-auto">
      ...
      >
        <div class="w-[50px] min-w-[50px] h-[50px] rounded-md">
          <!-- Null kontrolü -->
          <LazyImg class="w-full h-full object-cover rounded-md" :src="item.photos" alt="" />
        </div>
        <div class="flex flex-auto flex-col w-[30%]">
          ...
          <p>
            <!-- Null kontrolü -->
            <span>{{ store.state.cart.totalPrice }}</span> items - {{ item.name }}
          </p>
        </div>
        <p class="text-sm whitespace-nowrap font-bold text-gray-700">{{ item.price }} TL</p>
      </div>
    </div>
    <div class="border-b-[1px] mt-3"></div>
    <!-- btn checkout -->
